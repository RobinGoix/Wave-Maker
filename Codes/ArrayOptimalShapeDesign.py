...
and arrays manipulations to move the object.
"""
...
#Parameters
set_log_level(ERROR) #Output
Nx = 10 #Default 35
Ny = 5 #Default 22
delta_t = 0.5 #[s] Default 0.03 
dt_scaled = False #Put False and delta_t = 0.5 to run a Test
...
save_opt = False #Save the optimization results
ploting = True
transport_order = 1 #CG finite element degree for the transport equation (1 or 2).
...
def main(zeta_0, delta_t, annotate=True, ploting=False):
    ...
    while (t <= end):  
        ...
        traj = Vmax*exp(-4./(lambda0/c0*t + 0.05))
...
if __name__ == "__main__":
    ...
    print 'zeta_initial_type:', type(zeta_initial)
    zeta_initial = project(zeta_initial*shape_lb + 0.9*shape_ub,Q)
    
    zeta_0 = zeta_initial.vector()
    
    shape = [ScalarParameter(Constant(zeta_0[i])) for i in range(len(zeta_0))]  #TO BE CHANGED
    shape_lb = shape_lb.vector()
    shape_ub = shape_ub.vector()
    
    #Computation of the functional
    eta = main(zeta_0, delta_t, ploting=ploting)
    if(Norme=='L2'):
    ...
    if(Opt_Method=='pyipopt'):
        ...
        nlp = jhat.pyipopt_problem(bounds=(shape_lb, shape_ub))
   ...
   #Debuging
    ...
    def Jhat(zeta_initial): # the functional as a pure function of nu
        eta = main(zeta_initial, delta_t, False)
        return assemble(-Norme_scaling*(inner(eta,eta)+inner(grad(eta),grad(eta)))*dx_(1))
