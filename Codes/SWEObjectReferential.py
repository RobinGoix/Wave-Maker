...
U = (hd*g)**(0.5) #Speed
vObject = Expression(("U","0.0"), U=U)

seabed = 'hd'
movingObject = '- 0.25*1/pow(cosh(pow(3*0.5,0.5)*x[0]/2),2)'
...
def Input_boundary(x, on_boundary):
        return on_boundary and x[0] > x1 - DOLFIN_EPS - 1
    
No_Slip1 = DirichletBC(E.sub(1),  Expression("0.0"), Input_boundary)
...
F = 1./dt*inner(u-u_prev,v)*dx + inner(grad(u)*(u-vObject),v)*dx \
    - g*div(v)*eta*dx

#F += sigma**2.*1./dt*div(h*(u-u_prev))*div(h*v/2.)*dx \
#      - sigma**2.*1./dt*div(u-u_prev)*div(h*h*v/6.)*dx

F += 1./dt*(eta-eta_prev)*xi*dx - inner((h+eta)*(u-vObject),grad(xi))*dx
