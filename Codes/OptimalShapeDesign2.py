...
bc_zeta = DirichletBC(E.sub(2), 0.0, Dirichlet_Boundary())

bcs = [bc_X_u, bc_Y_u, bc_X_eta, bc_zeta] 
...
F += zeta_t*xi*dx - epsilon*inner(grad(xi),U_alpha)*zeta_*dx - epsilon*delta_t/2.*inner(grad(xi),U_t)*zeta_alpha*dx \
...
while (t <= end):  
    ...
    if (ploting==True):
        if(t<=5*delta_t/2.):
            ...
            VizZ = plot(zeta_,rescale=True, title = "Free Surface")
        else:
            VizE.plot(eta_)#,rescale=True, title = "Free Surface")
